$ ->
  setTimeout (->
    ...
    pumpSource.addEventListener 'pump_status_update', (e) ->
      ...
      row_id = 'pump-status-' + status.equipment.rhizome.particle_device.device_id + '-' + status.pump_id
      # Build the row's HTML.
      html = """
<tr id="#{row_id}">
  <td>#{status.equipment.rhizome.name}</td>
  ...
  <td>#{status.speed}</td>
    ...
    tempSource.addEventListener 'pump_status_update', (e) ->
      ...
      status = JSON.parse(JSON.parse(e.data))
      # Figure out the unique ID of the status update row.
      row_id = 'temperature-sensor-status-' + status.equipment.rhizome.particle_device.device_id + '-' + status.temp_id
      # Build the row's HTML.
      html = """
<tr id="#{row_id}">
  <td>#{status.equipment.rhizome.name}</td>
  ...
  <td>#{status.last_read_time}</td>
      ...
      return
    return
    heatEleSource = new EventSource('/status_updates/heats')
    heatEleSource.addEventListener 'heating_element_status_update', (e) ->
      # Get the status update into JSON format. For some reason we need to parse it twice.
      status = JSON.parse(JSON.parse(e.data))
      # Figure out the unique ID of the status update row.
      row_id = 'heating-element-status-' + status.equipment.rhizome.particle_device.device_id + '-' + status.heat_id
      # Build the row's HTML.
      html = """
<tr id="#{row_id}">
  <td>#{status.equipment.rhizome.name}</td>
